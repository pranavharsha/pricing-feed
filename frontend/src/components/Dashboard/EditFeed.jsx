...
import {editFeedsService} from "../../services/dashboardService";

function EditFeed({ setShowModal, activeFeed, getFeedsCallback }){
  const [selectedDate, setSelectedDate] = useState(new Date(activeFeed.date));
  
  const editFeedsData = async (data, id) => {
    const response = await editFeedsService(data, id);
  ...
  const formik = useFormik({
    initialValues: {
      store_id: activeFeed.store_id,
      sku: activeFeed.sku,
      product_name: activeFeed.product_name,
      price: activeFeed.price,
    },
    ...
    onSubmit: values => {
      ...
      if(selectedDate && activeFeed && activeFeed.id){
        editFeedsData(values, activeFeed.id);
      }
    },
  });
